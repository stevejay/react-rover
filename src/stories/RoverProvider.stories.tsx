...
import { RoverProvider, useRoverContainer, useRoverTabStop } from '@/RoverProvider';
...
const GridButton: FC<GridButtonProps> = ({ item }) => {
  const { ref, onClick, tabIndex } = useRoverTabStop(item);
  return (
    <MemoizedGridButton
      ref={ref}
      disabled={item.id === '5'}
...
const GridOfButtons: FC<GridOfButtonsProps> = ({ items, columnsCount }) => {
  const { onKeyDown } = useRoverContainer();
